...
3. **Miqyaslana bilənlik (Scalability):** Platformanın arxitekturası 1.8 milyon istifadəçiyə qədər şaquli və üfüqi miqyaslanmağa imkan verir
4. **Əlçatanlıq:** Mobil cihazlar, planşetlər və masa üstü kompüterlər üçün uyğunlaşdırılmış cavabverici (responsive) istifadəçi interfeysi
...
- **Təqdimat Qatı (Presentation Layer):** React 18 əsaslı tək səhifəli tətbiq (Single Page Application, SPA)
...
2. React tətbiqi başladılır, Supabase JS müştəri tətbiqi başladılır
...
|---|---|---|
| React | 18.x | Əsas istifadəçi interfeysi çərçivəsi |
...
|---|---|---|
| Supabase | Pro Plan | Arxa tərəf xidməti (Backend-as-a-Service, BaaS) |
...
- Yaradılma və yenilənmə zaman damğaları (timestamp) bütün cədvəllərdə tətbiq olunur
- Xarici açar (foreign key) əlaqələri istinad bütövlüyünü (referential integrity) təmin edir
...
| `school_type` | ENUM | NOT NULL | dövlət / xüsusi / lisey / gimnasiya |
...
Zirva platformasında beş əsas istifadəçi rolu tətbiq olunur:
...
- **Mesaj bildirişləri:** Yeni mesaj gəldikdə alıcı real vaxt bildirişi alır
...
graph TB
    subgraph "Zirva Platforması"
        T[Müəllim İstifadəçi İnterfeysi]
...
| Tədris dili | Azərbaycan dili əsas | Çoxdilli |
...
Platforma kurikulum tipinə görə istifadəçi interfeysi elementlərini uyğunlaşdırır:
...
- **Dil:** Milli üçün Azərbaycan dili istifadəçi interfeysi; IB üçün ikidilli (Azərbaycan + İngilis) seçim
...
Zirva platforması Supabase tərəfindən avtomatik yaradılan PostgREST REST API-sindən istifadə edir. Bütün REST sorğuları `https://[supabase-proje-id].supabase.co/rest/v1/` ünvanı ilə başlayır.
...
WebSocket əsaslı abunəlik `wss://[supabase-proje-id].supabase.co/realtime/v1` ünvanı vasitəsilə həyata keçirilir.
...
- Əsas qovşaq: iad1 (ABŞ Şərq); Avropa qovşağı: Frankfurt
...
- **Maksimum hovuz ölçüsü:** Supabase Pro Plan standart konfiqurasiyası ilə müəyyən edilir
- **Faydası:** 150,000 eyni vaxtda istifadəçini 100–300 actual PostgreSQL bağlantısına endirir

### 11.3 CDN Miqyaslanması

Vercel CDN-in statik fayl xidməti sonsuz miqyaslanma imkanı verir. React tətbiqinin bütün faylları (JS, CSS, HTML) CDN edge qovşaqlarında keşlənir. Azərbaycan istifadəçiləri üçün ən yaxın qovşaq Frankfurt (EU) qovşağıdır.
...
>
> Hazırda Zirva platformasının verilənlər bazası Supabase tərəfindən Amazon Web Services (AWS) **ap-northeast-2** regionunda, yəni **Seul, Cənubi Koreya**da yerləşdirilmişdir.
>
...
| Mərhələ 4 | Nazirlik serverindən idarəetmə (ixtiyari) | Nazirlik qərarına əsasən müəyyənləşdiriləcək |
...
| 2 | **Azərbaycanda yerli hosting:** Nazirliyin serverinin texniki tələbləri, SLA standartları və vaxt çizelgesi | ✓ Qərar tələb olunur |
