...
Bu sənəd Zirva Məktəb İdarəetmə Platformasının təhlükəsizlik arxitekturasını ətraflı şəkildə təsvir edir. Sənəd Azərbaycan Respublikası Elm və Təhsil Nazirliyinin № VM26005443 (24.04.2026) tarixli müraciətinə cavab olaraq, texniki şəffaflıq prinsipinə uyğun olaraq hazırlanmışdır.
...
**Dürüstlük qeydi:** Çox Faktorlu Autentifikasiya (Multi-Factor Authentication — MFA) texniki olaraq planlaşdırılmış, lakin hali-hazırda tətbiq edilməmişdir. Bu, pilot mərhələsinin ilk aylarında tamamlanacaqdır. Bu məsələ Bölmə 3-də ətraflı açıqlanmışdır.

**Kritik məlumat:** Verilənlər bazası hal-hazırda Cənubi Koreyada (AWS ap-northeast-2) yerləşdirilmişdir. Bu, məlumat rezidentliyi baxımından aradan qaldırılması planlaşdırılan bir riskdir. Ətraflı məlumat üçün ZRV-MSEA-2026-003 nömrəli sənədə müraciət edilməsi xahiş olunur.
...
İstifadəçilər e-poçt ünvanı və şifrə ilə giriş edə bilər. Şifrə tələbləri:

- Minimum 8 simvol uzunluğu
- Böyük hərf tələbi: [DOLDURUN: tətbiq edilib/edilməyib]
- Rəqəm tələbi: [DOLDURUN: tətbiq edilib/edilməyib]
- Xüsusi simvol tələbi: [DOLDURUN: tətbiq edilib/edilməyib]
...
- Tək istifadə prinsipi (single-use): hər istifadə yeni token cütü yaradır
- Müddət: 7 gün (konfiqurasiya edilə bilər)

### 2.4 Sehrli Keçid (Magic Link) Autentifikasiyası

Şifrəsiz giriş seçimi olaraq sehrli keçid (Magic Link) funksionallığı mövcuddur. İstifadəçi e-poçtunu daxil edir, Supabase Auth bir dəfəlik keçid göndərir. Bu keçid:

- 24 saat ərzində istifadə edilməlidir
- Yalnız bir dəfə istifadə oluna bilər
- Geçerli olmayan keçidlər avtomatik silinir
...
Cost faktoru 12 seçimi: bu, müasir donanımda təxminən 250ms hesablama vaxtı tələb edir — istifadəçi üçün qeyri-hiss edilən, lakin kütləvi brute-force hücumlarını praktiki olaraq qeyri-mümkün edən tarazlıq.
...
Bu, məqsədli texniki qərar deyil — pilot mərhələsinin sürətli inkişafı prioritetlərindən irəli gəlir. Zirva rəhbərliyi bu çatışmazlığı qəbul edir və Nazirliyin diqqətinə çatdırır.
...
4. Bərpa kodları (backup codes) generasiyası
...
1. **Tətbiq qatı RBAC:** React interfeysi rol əsaslı marşrut qoruması tətbiq edir
...
Giriş
  ...
  → refresh_token (7 gün müddətli, single-use)
...
Refresh token döndürmə (rotation) aktiv edilmişdir. Bu, köhnə refresh token oğurlandığı halda kötü niyyətli şəxsin onu istifadə edə bilməməsini təmin edir — çünki hər istifadə yeni token cütü yaradır və köhnəni etibarsız edir.
...
3. Müştəri tərəfindən yerli yaddaşdan (localStorage) token məlumatları silinir
4. İstifadəçi giriş səhifəsinə yönləndirilir

### 5.4 Paralel Sessiyalar

Hal-hazırda eyni istifadəçi hesabı ilə çoxsaylı paralel sessiya mümkündür (məsələn, həm telefon, həm kompüter). Bu davranış Supabase Auth-un standart konfiqurasiyasıdır. Pilot mərhələsindən sonra Nazirliyin tələbinə əsasən sessiya sayı məhdudlaşdırıla bilər.
...
- Müştəri tərəfindəki JavaScript koduna daxil edilir (public)
...
PostgREST parametrli sorğu (parameterized query) arxitekturası tətbiq edir. Bütün istifadəçi daxiletmələri avtomatik olaraq saniyələşdirilir. SQL injeksiya hücumları arxitektura səviyyəsindəmitirasiya edilmişdir.
...
| GDPR DPA | Mövcuddur (imzalanmalıdır) |

### 7.3 Şəbəkə Təhlükəsizliyi

- **Şəxsi Şəbəkə (Private Networking):** Supabase komponetləri (verilənlər bazası, auth, edge) AWS VPC (Virtual Private Cloud) içərisindədir
...
Bütün müştəri-server kommunikasiyası TLS ilə şifrələnir:
...
Bu, brauzerlərin həmişə HTTPS istifadə etməsini məcbur edir.
...
[DOLDURUN: şifrələnmiş sütunların siyahısı] sütunları üçün tətbiq qatında əlavə şifrələmə tətbiq etmək nəzərdə tutulur.
...
Git tarixçəsinin müntəzəm olaraq `git-secrets` və ya oxşar alətlərlə skan edilməsi nəzərdə tutulur. Hal-hazırda: [DOLDURUN: skan mövcuddurmu?]
...
| 4 | Daxili təhdid (işçi) | Aşağı | Yüksək | Minimum lazım olan giriş, audit jurnalı, role ayrılması |
...
Mərhələ 1 — Aşkarlanma
  ...
  → Audit jurnalı anaomaliyası
  → Məsul şəxs: [DOLDURUN: NOC/DevOps növbəsi]
...
Mərhələ 5 — Aradan Qaldırma
  → Köküsahə (root cause) müəyyən edilir
...
| İkincil texniki məsul | [DOLDURUN: CTO/DevOps] | [DOLDURUN] |
| Nazirlik əlaqə nöqtəsi | [DOLDURUN: VM26005443 müəllifi] | [DOLDURUN] |
...
sequenceDiagram
    participant R as React Tətbiqi
    participant SJ as Supabase JS Klient
    participant SA as Supabase Auth
...
| № | Məsələ | Növ |
|---|---|---|
| 1 | **MFA məcburiliyi:** Pilot proqramında inzibati istifadəçilər üçün MFA məcburi edilsinmi? Bu, pilot zaman çizelgəsini 4 həftəyə qədər uzada bilər | Qərar tələb olunur |
| 2 | **Sessiya müddəti:** Nazirlik standartlarına görə maksimum sessiya müddəti nə qədər olmalıdır? (Hal-hazırda: 1 saat access + 7 gün refresh) | Tələb məlumatı |
